...
if __name__ == "__main__":
    ...
    batch_size = 32
    _, test_dataset = data_gen(batch_size)
    ...
    with torch.no_grad():  # Disable gradient computation
        for state_seq, action_seq, skill_seq in test_loader:
            ...
            logits, _, intention_logits_for_smooth_loss, transition_logits, internal_index = model(action_dummy,
                                                                                                    state_seq,
                                                                                                    skill_seq)
            ...
            all_skill_index.append(skill_seq.cpu().numpy())
            all_internal_index.append(internal_index.cpu().numpy())
    ...
    for i in range(len(all_outputs)):
        print(
            f"  Target: {all_targets[i]}\n  Output: {all_outputs[i]}\ninternal: {all_internal_index[i]}\n   skill: {all_skill_index[i]}\n")